...
// This function fetches from YouTube API every 24 hours and caches in Firestore
...
const YOUTUBE_API_KEY = process.env.YOUTUBE_API_KEY || "AIzaSyARe_LDFt0r5iaS4qZ93fufs5CSOEHwY4";
...
export const fetchYouTubeVideosScheduled = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      // Update cache status to indicate error
      await db.collection("youtubeCache").doc("latest_videos").update({
        status: "error",
...
// Optional: HTTP endpoint to manually trigger fetch
export const fetchYouTubeVideosManual = functions.https.onRequest(
  async (req, res) => {
    try {
      const result = await fetchYouTubeVideosScheduled.run({} as any);
